...
- Form create/edit di dashboard menulis via REST API (`src/lib/admin-api.ts`) sehingga access control + hooks (`revalidateSite`) tetap jalan; halaman list (dan media) me-render halaman pertama via Local API di server components (instance Payload sudah di-cache global oleh `getPayload`) lalu mengoper `initialData`/`initialRowCount` ke tabel — mount TIDAK fetch ulang (data SSR sudah fresh; menghindari duplikasi query). Client hanya fetch REST saat search/sort/pagination dan setelah delete (tabel menandai seed `stale` agar remount fetch data fresh, bukan menampilkan baris yang sudah dihapus). Query REST tabel pakai `depth: 0` agar response kecil (tabel hanya menampilkan field scalar). Guard role manual tetap di halaman users.
- **Optimasi loading**: `getCurrentUser` dibungkus React `cache()` agar `payload.auth` hanya 1× per request (layout + page berbagi hasil); dashboard home memakai `<Suspense>` agar header tampil dulu. Jangan tambahkan singleton `getPayload` sendiri — Payload 3.87 sudah cache instance di `globalThis._payload`.
